...
use crate::{prelude::*, FontSheet, FrameFuture, KeyTyped, FRAME_DURATION};
...
pub fn run(_window_title: &str, amain: impl Future<Output = ()> + 'static) {
    ...
    loop {
        let dt = runtime::with(|r| r.elapsed_since_last_update());
        let n = (dt / FRAME_DURATION.as_secs_f64()) as u32;

        if n > 0 {
            runtime::with(|r| {
                r.logical_frame_count = n;
                r.last_update += n as f64 * FRAME_DURATION.as_secs_f64();
            });
